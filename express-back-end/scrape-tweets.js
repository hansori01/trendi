...
const pushToTweetsData = function(tweet) {
  ...
  try {
    let tweetData = {
      ...
      extendedTweet: tweet.extended_tweet.full_text,
      place: tweet.place,
      userLocationCoords: tweet.user_location_coords,
      sentiment: {
...
<<<<<<< HEAD
          const userLocation = tweet.user.location.replace(regex, '');
          getLatLngFromLocation(userLocation).then((location) => {
            tweet['user_location_coords'] = location;
=======
...
const getTweetsFromPointRadius = function (pointRadius) {
  T.get(
    "search/tweets",
    { q: `#RemoveThePM geocode:${pointRadius}`, count: 10 },
...
const runSingleQuery = function (hashtag) {
  ...
  needle.get(
    `https://api.twitter.com/1.1/search/tweets.json?q=%23${hashtag}%20-filter%3Aretweets%20AND%20-filter%3Areplies&geocode=${calgaryPointRadius}`,
    { headers: headers },
